                ...
                <form method="GET" action="{{ route('offre') }}" class="filter-form">
                    <div class="filter-grid">
                        ...
                        <select name="localisation" class="form-select location-select">
                            ...
                            @foreach ($offres->unique('localisation') as $offre)
                                <!-- unique() : method to get unique localisation values pour eviter la repetition -->
                                <option value="{{ $offre->localisation }}"
                                    {{ request('localisation') == $offre->localisation ? 'selected' : '' }}>
                                    {{ $offre->localisation }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="duration" class="form-select duration-select">
                            ...
                            @foreach ($offres->unique('duration') as $offre)
                                <option value="{{ $offre->duration }}"
                                    {{ request('duration') == $offre->duration ? 'selected' : '' }}>
                            ...
                            <div class="card-footer">
                                <div class="d-flex justify-content-between align-items-center date-container">
                                    <div class="date-item">
                                        ...
                                        <span
                                            class="date-value">{{ \Carbon\Carbon::parse($offre->date_debut)->format('d/m/Y') }}</span>
                                    </div>
                                    <div class="date-item">
                                        <i class="fas fa-flag-checkered text-success"></i>
                                        <span class="date-label">End Date</span>
                                        <span
                                            class="date-value">{{ \Carbon\Carbon::parse($offre->date_fin)->format('d/m/Y') }}</span>
                                    </div>
                                </div>

                                <div class="d-flex justify-content-between align-items-center mt-3 action-buttons">
                                    <a href="{{ route('offres.show', $offre->offre_id) }}" class="btn btn-details">
                        ...
                        <ul class="pagination mb-0">
                            ...
                            @for ($i = 1; $i <= $offres->lastPage(); $i++)
                                <li class="page-item {{ $i === $offres->currentPage() ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $offres->url($i) }}"
                                        data-page="{{ $i }}">{{ $i }}</a>
                                </li>
                            @endfor
